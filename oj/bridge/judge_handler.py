...
#from oj import event_poster as event
from oj.bridge.base_handler import ZlibPacketHandler, proxy_list
#from oj.caching import finished_submission
from oj.models.runtime import Language, Judge, RuntimeVersion
from oj.models.submission import Submission, SubmissionTestCase
from oj.models.problem import Problem #LanguageLimit, , RuntimeVersion, 
...
class JudgeHandler(ZlibPacketHandler):
    ...
    def get_related_submission_data(self, submission):
        ...
        try:
            s=Submission.objects.filter(id=submission).first()
            pid, time, memory, lid, sub_date, uid = [s.problem.code, s.problem.time_limit, s.problem.memory_limit, s.language.key, s.date, s.user.id]
        ...
        attempt_no = Submission.objects.filter(problem__code=pid, user__id=uid,
                                               date__lt=sub_date).exclude(status__in=('CE', 'IE')).count() + 1
